...
!function($s) {
  ...
  class DelayDevice extends $s.DeviceController {
    constructor(device) {
      super(device);
      var dev = this;
      dev._delay = Math.max(0, device.deviceDef.delay || defaultDelay);
      dev._color = device.deviceDef.color || defaultDelayColor;
      dev._state = device.deviceDef.state || { direction : Direction.WE };
      dev._in1 = dev.addInput();
      dev._out1 = dev.addOutput();
      connectNode(dev._in1, dev._out1, dev._delay);
    }
    ...
    createUI() {
      ...
      var $path = $s.createSVGElement('path').
        ...
        addClass('simcir-connector').css('stroke', dev._color);
      dev.$ui.append($path);
      ...
      dev.doc = {
        ...
        code: '{"type":"' + dev.deviceDef.type + '","delay":50}'
      };
    }
  }

  $s.registerDevice('Delay', DelayDevice);
